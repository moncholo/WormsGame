...
void RecorredorDeImagenes::inicializarZoomYRectangulo(){
	this->zoom = 0.75* ZOOM_MAX; //1 es el default, la imagen original
	
	this->rectanguloRecorredor = new SDL_Rect();
	this->rectanguloRecorredor->h = this->altoDeLaImagen;
	this->rectanguloRecorredor->w = this->anchoDeLaImagen;

	this->rectanguloRecorredor->x = this->anchoDeLaImagen/2;
	this->rectanguloRecorredor->y = this->altoDeLaImagen/2;
...
void RecorredorDeImagenes::procesarMovimientoADerecha(){
	...
	if ((this->anchoDeLaImagen-posicionXFinalDelRectangulo) > DESPLAZAMIENTO_MAX/this->zoom){
			this->rectanguloRecorredor->x += DESPLAZAMIENTO_MAX/this->zoom;
...
void RecorredorDeImagenes::procesarMovimientoAIzquierda(){
	float posicionXInicialDelRectangulo = this->rectanguloRecorredor->x;
	if (this->rectanguloRecorredor->x >= DESPLAZAMIENTO_MAX/this->zoom)
		this->rectanguloRecorredor->x -= DESPLAZAMIENTO_MAX/this->zoom;
	if ((this->rectanguloRecorredor->x< DESPLAZAMIENTO_MAX/this->zoom)||(this->rectanguloRecorredor->x < 0)){
		this->rectanguloRecorredor->x = 0;
	}
}
...
void RecorredorDeImagenes::procesarMovimientoAbajo(){
	...
	if ((this->altoDeLaImagen - posicionYFinalDelRectangulo) > DESPLAZAMIENTO_MAX/this->zoom){
		this->rectanguloRecorredor->y += DESPLAZAMIENTO_MAX/this->zoom;;
	}
	else if (((this->altoDeLaImagen-posicionYFinalDelRectangulo)<=DESPLAZAMIENTO_MAX/this->zoom) ||
...
void RecorredorDeImagenes::procesarMovimientoArriba(){
	...
	if (this->rectanguloRecorredor->y > DESPLAZAMIENTO_MAX/this->zoom){
		this->rectanguloRecorredor->y -= DESPLAZAMIENTO_MAX/this->zoom;
		}	
	else if (this->rectanguloRecorredor->y <= DESPLAZAMIENTO_MAX/this->zoom)
		this->rectanguloRecorredor->y = 0;
